...
type-check:
	uv run mypy --explicit-package-bases agy_swarms scripts/disk_space_preflight.py scripts/release_health.py scripts/release_health_registry.py scripts/release_health_docs.py scripts/rewrite_release_health_docs.py scripts/verify_release_tag.py
...
pr-verification:
	uv run python scripts/pr_verification.py --pr "$${PR_NUMBER:?set PR_NUMBER}" --pytest-count "$${PYTEST_COUNT:-747}" --mypy-files "$${MYPY_FILES:-100}" --release-health-passed "$${RELEASE_HEALTH_PASSED:-24}" --release-health-total "$${RELEASE_HEALTH_TOTAL:-24}"
